...
## The shared claim-reward ceremony (also used by the Daily Login bonus screen).
...
## Claim ceremony — reveal the reward card, then burst doobers into the currency
## bar (wallet credits + pulses as they land). Delegates to the shared
## MbRewardCeremony so the Daily Login bonus screen runs the identical animation.
