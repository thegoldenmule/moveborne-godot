...
## Self-update install runner for ui_kit. A Node parented OUTSIDE the plugin (so it
## survives set_plugin_enabled(false)) that owns the install-and-reload sequence —
## disable the plugin, extract the addon subtree from the downloaded repo archive,
## wait for a filesystem scan, then re-enable the plugin so the editor reloads the
## new scripts.
##
## Only files under `addons/ui_kit/` in the archive are touched: the repo's
## top-level files and everything else in the consuming project are left alone.
## Extract overwrites + adds; it never prunes, so a file removed upstream lingers
## until deleted by hand.
##
## Each file is written via `.tmp` + atomic rename with a per-file backup, and any
## mid-batch failure rolls every already-written file back to its previous
## contents. If the rollback ITSELF can't fully restore (FAILED_MIXED), the runner
## refuses to re-enable the plugin and leaves it disabled — a failed update never
## loads a half-installed tree.
##
## Frame-waiting + call_deferred between every step keeps the runner off the stack
## while the filesystem scan reloads scripts (this very script is among the files
## overwritten); the reload lands between steps, never mid-method. Fields are kept
## untyped where they survive that scan.

const PLUGIN_CFG_PATH := "res://addons/ui_kit/plugin.cfg"
const INSTALL_BASE := "res://"
const ADDON_REL_PREFIX := "addons/ui_kit/"
const TEMP_SUFFIX := ".uik_update_tmp"
const BACKUP_SUFFIX := ".uik_update_backup"
...
## promotes the outcome to FAILED_MIXED.
...
func _disable_plugin() -> void:
	print("ui_kit | update: disabling current plugin")
...
func _extract() -> void:
	...
	if status == InstallStatus.FAILED_MIXED:
		push_error(
			"ui_kit | self-update failed AND rollback could not restore the previous "
			+ "files. The plugin is left disabled — restore addons/ui_kit/ from git "
			+ "(or re-copy it from the repo), then re-enable the plugin. Look for "
			+ "*.uik_update_backup files to recover."
		)
		_wait_frames(POST_ENABLE_FRAMES, "_finish")
		return
	## FAILED_CLEAN: every written file was rolled back; bring the previous plugin
	## version back so the kit isn't left disabled.
	push_error("ui_kit | self-update failed; rolled back, re-enabling the current version")
...
func _do_extract() -> int:
	...
	if reader.open(zip) != OK:
		push_error("ui_kit | update: could not open %s" % zip)
	...
	# refusing an archive that maps two entries onto one target.
	var jobs := []
	var seen := {}
	var has_cfg := false
	...
	for entry in reader.get_files():
		...
		if rel.is_empty():
			continue
		if not _is_safe(rel):
			push_error("ui_kit | update: refusing unsafe path %s" % entry)
			reader.close()
			return InstallStatus.FAILED_CLEAN
		if seen.has(rel):
			push_error("ui_kit | update: archive maps two entries onto %s — aborting" % rel)
			reader.close()
			return InstallStatus.FAILED_CLEAN
		seen[rel] = true
	...
	if not has_cfg or not has_plugin:
		push_error("ui_kit | update: archive is missing plugin.cfg / plugin.gd")
		reader.close()
		return InstallStatus.FAILED_CLEAN
...
func _install_one(reader: ZIPReader, entry: String, rel: String, base: String) -> bool:
	...
	if DirAccess.make_dir_recursive_absolute(dir) != OK:
		print("ui_kit | update: could not create %s" % dir)
		return false
	...
	if f == null:
		print("ui_kit | update: could not write %s" % tmp)
		return false
	...
	if had_original:
		...
		if DirAccess.copy_absolute(target, backup) != OK:
			DirAccess.remove_absolute(tmp)
			print("ui_kit | update: could not back up %s" % target)
			return false
	...
	if DirAccess.rename_absolute(tmp, target) != OK:
		...
		if DirAccess.rename_absolute(tmp, target) != OK:
			...
			print("ui_kit | update: could not replace %s" % target)
			return false
	...
	while i >= 0:
		...
		if bool(rec["had_original"]):
			if not FileAccess.file_exists(backup):
				push_error("ui_kit | rollback: backup missing for %s" % target)
				ok = false
			else:
				DirAccess.remove_absolute(target)
				if DirAccess.copy_absolute(backup, target) != OK:
					push_error("ui_kit | rollback: could not restore %s" % target)
					ok = false
				else:
					DirAccess.remove_absolute(backup)
		else:
			if FileAccess.file_exists(target) and DirAccess.remove_absolute(target) != OK:
				push_error("ui_kit | rollback: could not delete %s" % target)
				ok = false
		i -= 1
	_written.clear()
	return ok
...
## Drop the per-file backups after a fully successful install. Best-effort.
...
## the addon. Accepts `addons/ui_kit/...` directly and the GitHub branch-archive
## shape `<wrapper>/addons/ui_kit/...` (exactly one leading directory segment), but
## rejects deeper nesting — so _is_safe()'s begins_with stays a real guard. Static
## so the verifier exercises it headless.
...
func _on_watchdog_timeout() -> void:
	...
	push_warning(
		"ui_kit | update: filesystem scan didn't confirm within %ds; proceeding"
...
func _enable_plugin() -> void:
	print("ui_kit | update: enabling the new plugin version")
	EditorInterface.set_plugin_enabled(PLUGIN_CFG_PATH, true)
	_wait_frames(POST_ENABLE_FRAMES, "_finish")
