...
## Self-update service for the ui_kit addon. The kit is committed into the
## consuming project (so a fresh clone works offline) but is *sourced* from a
## standalone repo, and this service checks that repo for a newer version and
## pulls it in place.
##
## Source of truth for "what version is upstream" is the repo's
## `addons/ui_kit/plugin.cfg` `version` on BRANCH — bumping it and pushing is what
## ships an update; no GitHub release ceremony required. The check reads that file
## raw; the install downloads the branch archive zip and extracts only the
## `addons/ui_kit/` subtree (see update_reload_runner).
##
## Holds NO Control / EditorInterface references, so it loads under
## `godot --headless`. The version-parse + compare helpers are static so a headless
## verifier exercises them with no editor and no network.
...
const REMOTE_CFG_URL := "https://raw.githubusercontent.com/thegoldenmule/godot-addons/main/addons/ui_kit/plugin.cfg"
...
const LOCAL_CFG_PATH := "res://addons/ui_kit/plugin.cfg"
const UPDATE_TEMP_DIR := "user://ui_kit_update/"
const UPDATE_TEMP_ZIP := "user://ui_kit_update/update.zip"
...
func _exit_tree() -> void:
	## If the editor tears the service down mid-download, drop the partial staging
	## file so it doesn't linger in user://. A successful update is in ST_INSTALLING
	## here, not ST_DOWNLOADING, so the zip the runner still needs is left in place.
...
func check_for_updates() -> void:
	...
	var err := _http.request(
		REMOTE_CFG_URL, ["Accept: text/plain", "User-Agent: ui_kit"]
	)
	if err != OK:
...
func start_update() -> void:
	...
	add_child(_dl)
	var err := _dl.request(ARCHIVE_URL, ["User-Agent: ui_kit"])
	if err != OK:
		## request_completed never fires when request() itself errors, so clean up
		## inline rather than leaking the HTTPRequest + a partial zip.
		_dl.queue_free()
		_dl = null
