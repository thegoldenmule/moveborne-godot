...
For this tutorial, lets use a game use case and assume we're building a game called **Snapser Battle Royale**. The game releases new characters, and the client needs an API to fetch if a character is available or not.
...
1. Scroll down to the **Main** section and add the following:
    ...
    - **Description**: List of available app characters
      ...
      "components": [
        {
          "id": "characters",
          "type": "textarea",
          "value": "" 👈 This is where the tool will store the value
        }
      ]
    }
  ]
...
- `Query Params`: `tool_id` and `environment`. Environment is 'dev' or 'stage' or 'prod'.
