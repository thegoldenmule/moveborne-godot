...
- [x] DEPLOY + SMOKE: Rebuild/publish the byosnap-validator image (build context = validator/) and roll with publish + snapend update (NOT sync). Smoke through the gateway with the snapser-validator skill: play a match to terminal state, GET currencies as the user and confirm increments; confirm a user-auth PUT is rejected (lockdown verification).
...
- [x] GODOT VERIFY: filesystem_manage reimport changed .gd paths; run editor McpTestSuite via godot-ai test_run; project_run + editor_screenshot to confirm the bar renders three balances and updates after an online Story match. Run one headless parity verifier as a sanity check (no logic/ files touched).
