...
- [ ] Follow-up (code-review, deferred): the claim_daily MbUi flow (goto:home -> press:home.daily -> press:missions.claim_all) races the sigil's async refresh — press opens the panel from a possibly-empty _quests_cache and Claim All only exists at >=2 claimable, so a headless driver can hit unknown_id. Make refresh awaitable (or have the flow wait until cards render) for reliable headless driving. Low impact: the user-facing tap path works; this only affects scripted runs (and the full claim path is anyway blocked on Snapser quest provisioning).
- [ ] Follow-up (code-review, tech debt, codebase-wide): MbQuestsClient._request / _error_text / the int64-string coercion / the [coins,souls,gems] CURRENCIES constant duplicate the same code already in leaderboards_client.gd / profile_client.gd / inventory_client.gd (a pre-existing pattern this feature follows). Extract a shared MbSnapserHttp helper (request + error_text + int64 parse) and a single currencies constant, then migrate all *_client.gd. Out of scope for this feature (touches 3 existing clients); track separately.
