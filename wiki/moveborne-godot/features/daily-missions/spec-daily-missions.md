...
Mission pool + rewards (ratified): a pool of 6 + 1 anchor, currency-only (coins), single-task recurring daily quests. Launch goals are COUNTER goals (client IncrementTaskProgress, a bounded daily mint); the match-derived ones graduate to validator statistics under the hardening brief. Anchor — Daily Dozen, play 3 matches, 100 coins (auto-assign). Pool — On a Roll (win 2, 150), Marathon (play 5, 150), High Roller (score 5,000 in a match, 120), Chain Reaction (5-tile merge, 120), Power Trip (use 2 power-ups, 80), Merge Master (merge 50 tiles, 100). Reward values are tunable via the Remote Config catalog. Provisioned + verified live on c4n1awfs. Pool size, the exact mission list, and per-mission reward values?

Four missions visible per day: the anchor + 3 rotating from the pool of 6 (a clean 2-day cycle), within the 4-8 target and reachable in one session. The static by_weekday map in the Remote Config daily_missions block names each UTC weekday's 3 pool missions; on open the client assigns that subset (AssignQuest) plus the auto-assigned anchor. How many missions visible per day — anchor + how many rotating (target 4–8 total)?

Counter-goal increment hook (built 2026-06-18, commit 0eb4923): hook point = match-exit. MbMatch tallies per-match stats (tiles merged, largest single-swipe merge from the engine's mergedTilesCount, power cards played) onto the match_exited result alongside score + story stars; MbDailyMissions.match_task_increments (pure, headless-tested) maps the finished match to per-task IncrementTaskProgress deltas; DailySigil.record_match_result applies them once per banked result (dm_recorded flag), live session only. Cumulative metrics (matches_played / matches_won / tiles_merged / powerups_used) add their amount; 'in a match' thresholds (match_score, merge_size) add the task goal once a single match meets it. Task names byte-match the snapend quest config (see the Snapend Provisioning node). Verified end-to-end on c4n1awfs. Counter-goal increment hook (the one remaining client integration): the Quests client now exposes increment_task(quest, task, delta), but WHICH gameplay results drive each launch counter mission, the exact Snapser task names, the hook point (match-exit from GameState.last_result vs live in-match), and how to model the threshold goals as counters (High Roller 'score 5,000 in a match', Chain Reaction '5-tile merge') are LiveOps/snapend decisions blocked on the Quests Settings provisioning (not yet on c4n1awfs). Proposed default once provisioned: increment at match-exit — play +1 always, win +1 when result.won, plus +1 for the threshold/action missions whose condition the match met (score>=5000, a >=5-tile merge occurred, power-ups used, tiles merged by count) — but the task names must byte-match the snapend config. Until then the assign/list/claim/UI loop is complete; counter goals simply won't advance. Confirm the action->task mapping + hook point at provisioning time.
