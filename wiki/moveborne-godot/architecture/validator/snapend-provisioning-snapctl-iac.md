...
Quest status lifecycle (verified live, commit 0b953a2 — a sharp gotcha). A daily quest moves assigned -> unclaimed -> completed: 'assigned' = in progress; 'unclaimed' = all tasks done and the reward is AVAILABLE (claimable); 'completed' = reward already claimed. The reward is NOT auto-granted on task completion — coins stay at 0 until an explicit ClaimQuestRewards, so the separate-claim UX is real. TRAP: the string 'unclaimed' literally contains the substring 'claimed', so the client must match these statuses EXACTLY (completed -> CLAIMED, unclaimed -> CLAIMABLE); an earlier substring test (status.contains('claimed')) read a freshly-claimable mission as already-claimed, so it looked auto-claimed and offered no Claim button. Fixed in MbDailyMissions.card_state + MbQuestsClient.is_claimable.

Snapend mutations invalidate live Snapser sessions (commit b311784). A snapend apply (this node's provisioning flow) or a byosnap sync (the deploy runbook) invalidates existing anonymous session tokens server-side. The token stays valid LOCALLY (anon TTL ~30 days), so a client that only checks local expiry keeps reusing a dead token: HTTP 401 on gateway reads (e.g. the story_progress blob -> 'progress unavailable') and the validator WS handshake rejected ('WS closed: -1'). The empty-progress fallback then also breaks story level-locking. Mitigation: MbSnapserAuth.reauth() does a fresh anon login KEEPING the persisted username (same user + progress), and the gateway clients retry once on a 401 (story_progress_client + remote_config_client). All clients share one MbSnapserAuth, so the first re-auth heals the token for the whole online surface, including the validator WS. Provisioning the daily-mission quests is exactly what triggered this in story mode.
...
- Daily-mission quest statuses are matched EXACTLY by the client: 'completed' = claimed (CardState.CLAIMED), 'unclaimed' = reward-available (CLAIMABLE) — never via substring, because "unclaimed" contains "claimed". Rewards require an explicit ClaimQuestRewards; they are NOT auto-granted on task completion.
- A snapend apply/sync invalidates live Snapser sessions; clients re-authenticate on a 401 (MbSnapserAuth.reauth — same username, so same user + progress) rather than trusting local token expiry alone (anon TTL ~30 days). The shared MbSnapserAuth means one re-auth heals all gateway clients + the validator WS.

## Synced commit
47ec744aa66a7520082c5f5eba6b05c847a01479
